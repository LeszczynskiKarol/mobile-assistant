...
function ResearchLog({ text }: { text: string }) {
  ...
  return (
    <View style={styles.researchLog}>
      ...
      {lines.map((line, i) => (
        <Text key={i} style={styles.researchLogLine}>{line}</Text>
      ))}
    </View>
  );
}
...
function ResponseText({ text, sources, onSourcePress }: {
  text: string;
  sources?: Source[];
  ...
  const parts: { type: "text" | "footnote"; content: string; index?: number }[] = [];
  const regex = /\[(\d+)\]/g;
  let lastIdx = 0;
  let match;
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    parts.push({ type: "footnote", content: match[0], index: parseInt(match[1]) });
        ...
        if (p.type === "footnote") {
          ...
          return (
            <Text
              ...
              onPress={() => source && (onSourcePress ? onSourcePress(source) : Linking.openURL(source.url))}
            >
              {p.content}
            </Text>
          );
  ...
  if (entry.type === "action" || entry.type === "error") {
    return (
      <View style={[styles.logItem, { borderLeftColor: colors[entry.type] }]}>
        <Text style={[styles.logText, { fontSize: 12, fontFamily: Platform.OS === "ios" ? "Menlo" : "monospace" }]}>
          {entry.text}
        </Text>
      </View>
    );
  ...
  return (
    ...
    >
      ...
      <ResponseText
        ...
        onSourcePress={(s) => setExpandedSource(expandedSource?.index === s.index ? null : s)}
      />
      ...
      {expandedSource && (
        ...
        >
          <Text style={styles.sourceTooltipTitle}>[{expandedSource.index}] {expandedSource.title}</Text>
      ...
      {showStats && entry.stats && (
        <View style={styles.statsDetail}>
          <Text style={styles.statsDetailText}>
            {entry.stats.inputTokens} in + {entry.stats.outputTokens} out = {entry.stats.totalTokens} tok
          </Text>
          <Text style={styles.statsDetailText}>
            ${entry.stats.costUsd.toFixed(5)} · {entry.stats.latencyMs}ms · {entry.stats.model}
          </Text>
        </View>
      )}
  ...
  return (
    <View style={styles.modelPicker}>
      {MODELS.map((m) => (
        ...
        >
          <Text style={[styles.modelBtnText, model === m.id && styles.modelBtnTextActive]}>
            {m.short}
          </Text>
        </Pressable>
      ))}
...
export default function VoiceScreen() {
  ...
  const {
    state, transcript, log, conversationId, model,
    setModel, toggle, newConversation, loadConversation,
    processText, voiceAvailable,
  } = useVoiceAssistant();
  ...
  useEffect(() => {
    if (params.conversationId) {
      getConversation(params.conversationId).then((data) => {
        loadConversation(data.id, data.messages.map((m) => ({
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
          ...
          <Pressable onPress={() => router.push("/conversations")} style={styles.historyBtn}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          ...
          {log.length === 0 && (
            <View style={styles.emptyContainer}>
              ...
              <View style={styles.examplesGrid}>
                {["Utwórz kartę w Trello", "Jakie są najnowsze modele Claude?", "Porównaj React vs Vue w 2026", "Co mam w kalendarzu?"].map((ex) => (
                  <Pressable key={ex} style={styles.exampleChip} onPress={() => { setTextInput(ex); }}>
          ...
          {(state === "processing" || state === "researching") && (
            <View style={styles.spinnerRow}>
              ...
              <Text style={styles.spinnerText}>
                {state === "researching" ? "Szukam informacji online..." : "Przetwarzam..."}
              </Text>
            </View>
          )}
        ...
        <View style={styles.textRow}>
          ...
          />
          <Pressable
            style={[styles.sendBtn, (!textInput.trim() || state === "processing" || state === "researching") && styles.sendBtnDisabled]}
            onPress={handleSendText}
            disabled={!textInput.trim() || state === "processing" || state === "researching"}
          >
        ...
        <View style={styles.buttonContainer}>
          <Pressable
            onPress={toggle}
            disabled={state === "processing" || state === "researching" || !voiceAvailable}
            style={({ pressed }) => [
              styles.voiceButton,
          ...
          >
            <Text style={styles.voiceButtonText}>{voiceAvailable ? config.label : "🎤"}</Text>
          </Pressable>
          <Text style={styles.stateLabel}>
            {!voiceAvailable && "Mikrofon tylko w APK"}
            {voiceAvailable && state === "idle" && `${model === "claude-sonnet-4-6" ? "Sonnet" : "Haiku"} · naciśnij żeby mówić`}
            {voiceAvailable && state === "listening" && "Słucham..."}
            {voiceAvailable && (state === "processing" || state === "researching") && "Przetwarzam..."}
...
const styles = StyleSheet.create({
  ...
  newBtn: {
    width: 30, height: 30, borderRadius: 8, backgroundColor: "#1e3a5f",
    justifyContent: "center", alignItems: "center",
  },
  newBtnText: { color: "#60a5fa", fontSize: 16, fontWeight: "700" },
  historyBtn: {
    width: 30, height: 30, borderRadius: 8, backgroundColor: "#1e293b",
    justifyContent: "center", alignItems: "center",
  },
  historyBtnText: { fontSize: 16 },

  // Model picker
  modelPicker: { flexDirection: "row", backgroundColor: "#1e293b", borderRadius: 8, padding: 2 },
  ...
  logItem: { borderLeftWidth: 3, paddingLeft: 12, paddingVertical: 8, marginBottom: 14 },
  logRole: { color: "#64748b", fontSize: 10, fontWeight: "600", marginBottom: 4 },
  ...
  footnote: {
    ...
    backgroundColor: "#06b6d420", paddingHorizontal: 2, borderRadius: 3,
  },

  // Source tooltip
  sourceTooltip: {
    backgroundColor: "#1e293b", borderRadius: 8, padding: 10, marginTop: 6,
  ...
  sourcesTitle: { color: "#94a3b8", fontSize: 11, fontWeight: "600", marginBottom: 6 },
  sourceItem: {
  ...
  // Research log
  researchLog: {
    backgroundColor: "#1e1b4b", borderRadius: 10, padding: 12, marginBottom: 12,
    borderLeftWidth: 3, borderLeftColor: "#8b5cf6",
  },
  researchLogTitle: { color: "#a78bfa", fontSize: 11, fontWeight: "600", marginBottom: 6 },
  researchLogLine: { color: "#c4b5fd", fontSize: 11, lineHeight: 18 },

  // Stats
  statsDetail: { marginTop: 6, paddingTop: 6, borderTopWidth: 1, borderTopColor: "#334155" },
  statsDetailText: { color: "#64748b", fontSize: 10, marginBottom: 2, fontFamily: Platform.OS === "ios" ? "Menlo" : "monospace" },

  // Spinner
  spinnerRow: {
    flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 12, paddingLeft: 4,
  },
  spinnerText: { color: "#a78bfa", fontSize: 13 },

  // Transcript
  transcriptInline: {
    paddingLeft: 14, paddingVertical: 6, borderLeftWidth: 2, borderLeftColor: "#06b6d440",
  },
  transcriptText: { color: "#06b6d4", fontSize: 14, fontStyle: "italic", opacity: 0.7 },

  // Empty state
  emptyContainer: { alignItems: "center", paddingTop: 50 },
  emptyIcon: { fontSize: 48, marginBottom: 8 },
  emptyTitle: { fontSize: 22, fontWeight: "700", color: "#f1f5f9", marginBottom: 6 },
  emptyText: { color: "#64748b", fontSize: 14, lineHeight: 22, textAlign: "center", paddingHorizontal: 20 },
  examplesGrid: { flexDirection: "row", flexWrap: "wrap", gap: 8, marginTop: 20, paddingHorizontal: 10 },
  exampleChip: { backgroundColor: "#1e293b", borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8, borderWidth: 1, borderColor: "#334155" },
  ...
  sendBtn: {
    width: 44, height: 44, borderRadius: 22, backgroundColor: "#0891b2",
    justifyContent: "center", alignItems: "center",
  },
  ...
  // Voice button
  buttonContainer: {
    alignItems: "center", paddingVertical: 16, paddingBottom: 28,
    borderTopWidth: 1, borderTopColor: "#1e293b",
  },
  voiceButton: {
